	...
	$.extend({
		edbox: function(settings, elem){
			...
			if(!edboxInstance){
				...
				options.overlayBg && boxOverlay.css('background', options.overlayBg);
				...
				if (content == null || content == '') {
					...
					if (!getWhiteSpace && !getId && !getAt && !getTag && getExt) {
						...
						boxLoad
							.append('<div id="' + options.prefixId + '-loading" />')
							.css({
				...
				function openBox() {
					var boxClose = box
						.prepend('<a href="#" id="' + options.prefixId + '-close"/>')
					...
					box.css({
						...
						'max-height': window_h - 40,
						'max-width' : window_w - 40
					});

					boxContent.css({
						'height': box.height() - (options.header && boxHeader.outerHeight()) - (options.footer && boxFooter.outerHeight())
					});

					if (boxContent[0].scrollWidth > Math.ceil(boxContent.width())) {
						boxContent.css('overflow-x','scroll');
					}

					if (boxContent[0].scrollHeight > Math.ceil(boxContent.height())) {
						boxContent.css('overflow-y','scroll');
						!options.header && boxClose.css('right', (window.innerWidth - $(window).width()) + parseInt(boxClose.css('right')));
					}
